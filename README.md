...
- #### Chapter 5:
 ...
 - #### Chapter 6:
    ...
    - We have to take care of normal and relative URLs.
    - We stored each node's property in one pass and then painted the node.
 - #### Chapter 7:
    - Browser -> ( Chrome + Tab ).
  ...
  - #### Chapter 10:
    ...
    - Cookie Jar is a global instance of a browser instance.
    - So one can place cross-site requests, we mitigate it by same origin policy.
    - Then we came across the issue of cross-site request forgery (CSRF) in which other sites can post a form of some other site.
    - On the server's backend, we introduced nonce which is just mapping each form generated by the server with a number and checking the nonce on submission.
    - For the browser side, a new attribute SameSite is introduced if the SameSite attribute is set then cross-site POST requests will be not allowed.
    - Another issue was malicious script posts, we introduced new header Content-Security-Policy which is followsd by origin URLs.
    - Request for the origin urls in CSP headers are only allowed in that tab.
### Fixes:
